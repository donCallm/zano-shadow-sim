...
impl NodeImplementation for CupratedImpl {
    ...
    fn render(&self, spec: &NodeLaunchSpec, _phase_args: Option<&Vec<String>>) -> RenderedLaunch {
        ...
        // NOTE: peer_args are monerod flag strings and cannot be consumed by
        // cuprate; peers are wired via the seed override in P3b (logical peers).
...
mod tests {
    ...
    fn cuprated_render_emits_fakechain_config() {
        ...
        assert!(toml.contains("network = \"FakeChain\""));
